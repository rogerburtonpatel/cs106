...
(* Todo: this case ^^ for easy reading *)
  fun exp (A.SIMPLE expr) = 
      (case expr
        ...
        | (A.FUNCODE (ns, e)) => S.LAMBDA (ns, exp e))

    | exp (A.IFX (a, e1, e2)) = S.IFX (S.VAR a, (exp e1), (exp e2))
      (* 𝓔⟦let x = e in x⟧ = 𝓔⟦e⟧ *)
    | exp (A.LETX (n, e, A.SIMPLE (A.NAME n'))) = 
          if n = n'
          then exp (A.SIMPLE e)
          else lt' n (exp (A.SIMPLE e)) (exp (A.SIMPLE (A.NAME n')))
    | exp (A.LETX (n, e1, e2)) = lt' n (exp (A.SIMPLE e1)) (exp e2)
    | exp (A.BEGIN (e1, e2)) = S.BEGIN [exp e1, exp e2]
    | exp (A.SET (n, e)) = S.SET (n, exp e)
    | exp (A.WHILEX (n, e1, e2)) = 
                    S.WHILEX ((lt' n (exp e1) (S.VAR n)), exp e2)
