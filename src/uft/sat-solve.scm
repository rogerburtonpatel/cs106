...
(define list-of? (A? v) 
    (if (null? v)
        ...
        (check-expect #t (list-of? atom? '()))
        (check-expect #t (list-of? atom? (list3 1 2 'a)))
        (check-expect #t (not (list-of? atom? (list3 o 2 'a))))
        (check-expect #t (list-of? function? (list3 car cdr o)))
        (check-expect #t (not (list-of? even? even?)))
        (check-expect #t (list-of? even? (cons 2 '())))
        (check-expect #t (not (list-of? atom? (cons 1 2))))
        (check-expect #t (not (list-of? atom? (cons 1 (cons 'a 2)))))
        (check-expect #t (not (list-of? atom? (cons '() (cons '() 2)))))
...
(define formula? (v)
    (if (symbol? v)
        ...
        ; null cases 
        (check-expect #t (formula? (make-and '()))) 
        (check-expect #t (formula? (make-or '()))) 
        (check-expect #t (not (formula? (make-not '()))))
        
        ; true cases for all  
        (check-expect #t (formula? 'x))
        (check-expect #t (formula? (make-not 'y)))
        (check-expect #t (formula? (make-and (list2 'y 'x)))) 
        (check-expect #t (formula? (make-or (list3 'y 'x 'z))))
        (check-expect #t (formula? (make-or (list3 (make-not 'x) 'x (make-and (list2 'y 'z))))))
        ; false cases for all
        (check-expect #t (not (formula? #t)))
        (check-expect #t (not (formula? (make-not #t))))
        (check-expect #t (not (formula? (make-and (list2 'y #f)))))
        (check-expect #t (not (formula? (make-or (list3 'y 'x #t)))))
        (check-expect #t (not (formula? (make-or (list3 (make-not 'x) 'x (make-and (list2 'y #t)))))))
...
(define eval-formula (f env)
    (if (symbol? f)
        ...
        (check-expect #t (eval-formula 'x test-env))
        (check-expect #t (not (eval-formula (make-not 'y) test-env)))
        (check-expect #t (eval-formula (make-and (list2 'y 'x)) test-env))
        (check-expect #t (eval-formula (make-or (list3 'y (make-not 'x) 'z)) test-env))

        (check-expect #t (not (eval-formula (make-or (list2 (make-not 'x)  (make-and (list2 (make-not 'y) 'z)))) test-env)))

        (check-expect #t (eval-formula (make-and '()) test-env))
        (check-expect #t (not (eval-formula (make-or '()) test-env)))
